...
def parse_batch(raw_bytes: bytes):
    ...
    for sample_str in text.split(";"):
        ...
        try:
            neck   = float(parts[0])
            arm    = float(parts[1])
            # Temperature fields are optional — fall back to the last known value
            t_neck = float(parts[2]) if len(parts) > 2 else latest["t_neck"]
            t_arm  = float(parts[3]) if len(parts) > 3 else latest["t_arm"]
        except ValueError:
...
async def ble_connect_and_stream(update_status):
    ...
    # Keep scanning until we find the device
    device = None
    while device is None:
        found_devices = await BleakScanner.discover(timeout=4.0)
    ...
    async with BleakClient(device.address) as client:
        ...
        # Hold the connection open until something sets connected = False
...
def start_ble_thread(update_status):
    ...
    def run():
        asyncio.run(ble_connect_and_stream(update_status))
